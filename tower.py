...
class TowerGroup:
    ...
    def get_click(self, x, y):
        ...
        if self.upgrade_menu is not None:
            for btn in self.upgrade_menu.get_buttons():
                if btn.clicked(x, y):       #有按鈕被點擊則回傳按鈕名稱
                    self.button_response = btn.response()
            if self.button_response is None:   #沒有按鈕被點擊則表沒有塔被選中
                self.selected_tower = None
    ...
    def upgrade(self):
        ...
        if self.selected_tower.level < 5:         #只要還可以升級，就把等級加一
    ...
    def sell(self):
        ...
        self.__towers.remove(self.selected_tower)  #將塔刪除
        self.selected_tower = None                 #塔被刪除所以沒有塔被選取
